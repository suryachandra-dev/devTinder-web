...
const Requests = () => {
  ...
  if (!requests || requests.length === 0) {
    return <div className="text-center text-xl font-semibold mt-10">No requests found</div>;
  }
    ...
    <div className="text-center my-10 px-4">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {requests.map((request) => {
          const { firstName, lastName, photourl, _id, about, age, gender } = request.fromUserId;
          return (
            <div
              key={_id}
            ...
            >
              {/* Image */}
              <img className="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover" src={photourl} alt="photo" />

              {/* Text Content */}
              <div className="flex flex-col ml-4 text-left flex-1">
                <h1 className="font-bold text-lg text-gray-900">{firstName + " " + lastName}</h1>
